...
topping_prices = dict(zip(toppings, map(formatted_price, prices)))
...
def topping_info(topping):
    ...
    return dict(topping=toppings[index], price=prices[index], formatted_price=formatted_price(prices[index]))
...
print([topping_info(topping) for topping in toppings])
